                   ...
                   <div key={idx} className={`bg-white p-5 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden ${isExpired ? 'ring-2 ring-rose-100' : ''}`}>
                       <div className="flex justify-between items-start mb-3">
                           <div className="flex items-center gap-3">
                               <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${category.color.replace('text-', 'bg-').split(' ')[0]} bg-opacity-20 text-${category.color.split('text-')[1]}`}>
                                   <CatIcon size={20} />
                               </div>
                               <div>
